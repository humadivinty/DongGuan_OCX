...
int BaseCamera::ConvertIDToStreamID(int iIndex)
{
    int iStreamID = 0;
    switch (iIndex)
    {
...
int BaseCamera::GetCamStatus()
{
    ...
    if (HVAPI_GetConnStatusEx((HVAPI_HANDLE_EX)m_hHvHandle, CONN_TYPE_IMAGE, &dwStatus) == S_OK)
    {
...
//    }
//#endif
    sprintf_s(chLogPath, sizeof(chLogPath), "C:\\H264CU_Log\\%04d-%02d-%02d\\",
        systime.wYear,
        systime.wMonth,
        systime.wDay);
    ...
    //    Last_Month,
    //    Last_Day);
    sprintf_s(chOldLogFileName, sizeof(chOldLogFileName), "%s\\%04d-%02d-%02d\\",
        chLogPath,
        Last_Year,
        Last_Month,
        Last_Day);
    ...
    sprintf_s(chLogFileName, sizeof(chLogFileName), "%s\\CameraLog-%d-%02d_%02d_%s.log",
        chLogPath,
        systime.wYear,
        systime.wMonth,
        systime.wDay,
...
bool BaseCamera::SetOverlayVedioFont(int iFontSize, int iColorR, int iColorG, int iColorB)
{
    char chLog[260] = { 0 };
    sprintf_s(chLog, sizeof(chLog), "SetOverlayVedioFont, size = %d, colorR = %d, colorG= %d, colorB= %d", 
        iFontSize, 
        iColorR, iColorG, iColorB);
    WriteLog(chLog);
    HRESULT hRet = S_FALSE;
    HRESULT hRet2 = S_FALSE;
    int iR = iColorR, iG = iColorG, iB = iColorB;
...
bool BaseCamera::SetOverlayVedioFont(int streamID, int iFontSize, int iColorR, int iColorG, int iColorB)
{
    char chLog[260] = { 0 };
    sprintf_s(chLog, sizeof(chLog), "SetOverlayVedioFont,streamID= %d, size = %d, colorR = %d, colorG= %d, colorB= %d",
        streamID, iFontSize, iColorR, iColorG, iColorB);
    WriteLog(chLog);
    HRESULT hRet = S_FALSE;
    int iR = iColorR, iG = iColorG, iB = iColorB;
    ...
    if (m_hHvHandle)
    {
        //int iStreamID = ConvertIDToStreamID(streamID);
        hRet = HVAPI_SetOSDFont((HVAPI_HANDLE_EX)m_hHvHandle, streamID, iFontSize, iR, iG, iB);
        if (S_OK == hRet)
        {
            WriteLog("set  Font  success.");
        }
    }
    else
    {
