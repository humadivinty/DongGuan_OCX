...
bool Tool_Img_ScaleJpg(PBYTE pbSrc, int iSrcLen, PBYTE pbDst, size_t *iDstLen, int DstWidth, int DstHeight, int compressQuality)
{
    ...
    int iDstWidth = DstWidth;
    int iDstHeight = DstHeight;
    ...
    {
        //use default
        iDstWidth = bmpSrc.GetWidth();
        iDstHeight = bmpSrc.GetHeight();
    }
    ...
    // 将位图按照JPG的格式保存到输出流中
    int iQuality = (compressQuality >= 100 ) ? 100 : compressQuality % 100;
...
void Tool_WriteLog(const char* chlog)
{
    ...
    //        systime.wDay);
    //}
    //sprintf_s(chLogPath, sizeof(chLogPath), "G:\\GitHub\\H264CU\\Debug\\");
    //sprintf_s(chLogPath, sizeof(chLogPath), "C:\\H264CU_Log\\");
    ...
    sprintf_s(chLogFileName, sizeof(chLogFileName), "%s\\%04d-%02d-%02d_%s",
        chLogPath,
        systime.wYear,
        systime.wMonth,
        systime.wDay,
...
int Tool_AnalysisPlateColorNo(const char* szPlateNo)
{
    ...
    else
    {
            ...
            )
        {
            ...
            {
                return COLOR_BLUE_WHIETE;
            }
            else if (NULL != strstr(szPlateNo, "蓝"))
            {
                return COLOR_BLUE;
            }
            else
