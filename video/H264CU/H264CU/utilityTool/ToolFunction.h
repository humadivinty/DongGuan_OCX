...
// Describe:        压缩图片到指定长宽大小
// FullName:      Tool_Img_ScaleJpg
// Access:          public 
// Returns:        bool
// Returns Describe:
// Parameter:    PBYTE pbSrc   ；输入参数， 源图片数据指针
// Parameter:    int iSrcLen        ；输入参数， 源图片数据长度
// Parameter:    PBYTE pbDst    ；输出参数， 输出图像的数据缓冲区
// Parameter:    size_t * iDstLen   ；输入输出参数，输入缓冲区大小，输出最终图片的实际大小
// Parameter:    int iDstWidth      ；输入参数，指定需要压缩的宽度
// Parameter:    int iDstHeight     ；输入参数，指定需要压缩的高度
// Parameter:    int compressQuality    ；输入参数，指定压缩的质量
//************************************
